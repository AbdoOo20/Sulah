...
///setters
  ...
  Future<ApiResponse> calculateOrderCost({required BuildContext context,required bool isDelivery}) async {
    _isLoading = true;
    notifyListeners();
    var cartItems = Provider.of<CartProvider>(context, listen: false).cartItems;
    List<ProductData> productList = [];
    for (var item in cartItems) {
      productList.add(ProductData(productId: item.id,qty: item.quantity,));
    }
    OrderBody calculateOrderCost = OrderBody(
      ...
      receiveType: isDelivery ? "delivery" :"pickup_from_store",// shipping,store
      product: productList,
    );
    ApiResponse responseModel = await ordersRepo.calculateOrderCostRepo(calculateOrderCost);

    if (responseModel.response != null &&
        responseModel.response?.statusCode == 200) {
      _costModel = CalculateOrderCostModel.fromJson(responseModel.response?.data);
      if (_costModel?.code == 200) {
        noteController?.text='';
        _paymentType=null;
        Provider.of<CartProvider>(context, listen: false).cartItems.clear();
        pushAndRemoveUntil(const MyOrders());
      }else {
